...
import { setItemCover, setItemProfile } from '../../../store/slices/sale';
import { getAllBySale, update, upload, remove } from '../../../store/slices/saleImage';
...
export default function TabImages(props: TabImageProps) {
    ...
    const { sale } = props;
    const { uploading, getting, updating, removing, error, items } = useSelector((state: RootState) => state.saleImage);
    /**
     * Busca todas as imagens da promoção
     */
    useEffect(() => {
        if (sale) {
