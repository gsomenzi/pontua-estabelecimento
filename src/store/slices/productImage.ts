...
import ImageService from '../../services/productImage';
...
export const getAllByProduct = createAsyncThunk(
    'productImage/getAllByProduct',
    async (id: number | string, thunkAPI: any) => {
        try {
            const { data } = await ImageService.getAllByProduct(id);
            return data;
        } catch (e) {
...
export const upload = createAsyncThunk('productImage/upload', async (payload: any, thunkAPI: any) => {
    try {
...
export const update = createAsyncThunk('productImage/update', async (payload: any, thunkAPI: any) => {
    try {
        const { id } = payload;
...
export const remove = createAsyncThunk('productImage/remove', async (id: string | number, thunkAPI: any) => {
    try {
...
export const slice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            // GETALL
            .addCase(getAllByProduct.pending, (state) => {
                state.getting = true;
                state.error = null;
                state.items = [];
            })
            .addCase(getAllByProduct.fulfilled, (state, action: PayloadAction<any>) => {
                state.getting = false;
                state.items = action.payload;
            })
            .addCase(getAllByProduct.rejected, (state, action: PayloadAction<any>) => {
                state.getting = false;
